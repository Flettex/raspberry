...
pub async fn post(
    ...
    if let Some(session_id) = id {
        let session_cookie: server::AuthCookie = serde_json::from_str(&session_id.id().unwrap()).unwrap();
        match db::verify::code(
        ...
        .await
        {
            Ok(code) => {
                ...
                if body.code == code.unwrap() {
                    ...
                    db::verify::delete_code(session_cookie.user_id, pool.as_ref()).await.unwrap();
